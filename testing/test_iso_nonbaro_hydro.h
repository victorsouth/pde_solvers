    ...
    const pde_solvers::iso_nonbaro_improver_pipe_PQ_task_boundaries_t& conditions)
{
    ...
    iso_nonbaro_improver_impulse_equation_t pipeModel(pipe, layer.density_std.value, layer.improver_concentration.value, conditions.volumetric_flow, euler_direction);
...
/// @brief Создает простую равномерную трубу c присадкой для тестовых расчетов
...
//    iso_nonbaro_improver_impulse_equation_t pipeModel(pipe, layer.density_std.value, layer.improver_concentration.value, initial_Q_for_Newton, euler_direction);
//
//    solve_condensate_PP<iso_nonbaro_improver_pipe_PP_task_boundaries_t, iso_nonbaro_improver_pipe_layer_t> test = solve_condensate_PP(pipeModel, conditions, layer);
...
//    //Arrange
//    auto pipe = create_test_pipe_for_PQ();
//    pde_solvers::iso_nonbaro_improver_pipe_PQ_task_t task(pipe);
//
//    std::vector<double> flows = { 0.1, 0.3, 0.5, 0.7 }; // м³/с
//
//

...
5. Чтобы определить высотные отметки соответствующих координат нового профиля применяется следующий метод:
  5.1. Для каждой точки определяется область притяжения - по половине сегмента в каждую сторону
  5.2. На области притяжения определяется максимальная высотная отметка и присваивается соответствующей координате
...
<br>
  ...
  5.3. Так как начальной и конечной точкам нового профиля будут соответствовать высотки начальной и конечной точек исходного профиля, область притяжения второй и последней точек полученного профиля будет составлять полтора сегмента

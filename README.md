...
**Простейшая задача на проблемно-ориентированный буфер**
...
1.	Использование moc_solver. 
	a.	Идея профиля промежуточных расчетов. 
	b.	Она раскрывается тут же. Есть целевые параметры, есть промежуточные расчетные параметры. Иногда промежуточные расчетные параметры одни и те же.
	c.	Задача – организация исходных данных в научных расчетах. Рассчитывается сразу много параметров. Плотность, вязкость, гидравлика.
2.	Обертка над буфером для численного метода

При использовании moc_solver также в слой целесообразно добавить профиль вспомогательных параметров. Расчетный слой такой задачи может выглядеть следующим образом

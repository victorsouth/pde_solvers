...
При работе с буферами целесообразно использовать концепцию проблемно-ориентированного слоя. В данном случае речь идет не об использовании имеющегося кода, а о рекомендациях, как целесообразно реализовывать код для конкретных расчетных задач.
Пусть расчетная задача состоит в моделировании движения партий нефти по трубе, причем свойства партий описываются плотностью и вязкостью. Наиболее естественным представлением слоя данных является структура, в которой имеются профили плотности и вязкости. При использовании `moc_solver` также в слой целесообразно добавить профиль вспомогательных параметров. Расчетный слой такой задачи может выглядеть следующим образом:
...
В данном случае рассматривается концепция, согласно которой расчётные алгоритмы не должны иметь представления о конкретной сущности решаемой задачи. Это означает, что метод характеристик не должен зависеть от того, проводится ли расчёт по плотности, вязкости или другим параметрам.
Однако структура данных, которая естественна для разработчика, может оказаться неудобной для расчётного алгоритма метода характеристик. В связи с этим возникает вопрос: откуда обобщённый расчётный алгоритм должен знать, что необходимо обратиться к полю слоя под названием `density` или `viscosity`?
Для решения этой проблемы была разработана обёртка `get_buffer_wrapper` совместно с `moc_layer_wrapper<1>`. Суть этой обёртки заключается в том, что вместо единственных целевых переменных `vars` у нас имеется несколько переменных, таких как плотность и вязкость.
Кроме того, доступна обёртка `moc_layer_wrapper<1>`, которая позволяет использовать плотность и вязкость из буфера в методе характеристик. Это позволяет сделать структуру данных более удобной для расчётного алгоритма и обеспечить его независимость от конкретной сущности решаемой задачи.
Процесс использования этих оберток выглядит следующим образом:
1.	Объявляется проблемно-ориентированный буфер.
2.	В буфер загружаются начальные условия для вязкости и плотности.
3.	Получается обертка для плотности с помощью `get_buffer_wrapper`.
4.	Объявляется расчетный алгоритм метода характеристик.
5.	Для расчетного алгоритма вызывается метод `step`.
6.	Аналогичные действия с шагов 3 по 5 выполняются для вязкости.
Этот подход позволяет отделить специфику задач от реализации расчетных алгоритмов, делая их более обобщенными и универсальными. В результате, расчетные алгоритмы могут работать с различными параметрами и сущностями, такими как плотность и вязкость, через обертки, которые обеспечивают прозрачное взаимодействие между различными компонентами системы.
Пример использования проблемно-ориентированного буфера для метода характеристик приведен Пример работы с временными рядами приведён в файле `test_moc.h` в тесте `MOC_Solver.UseCase_Advection_Density_Viscosity`

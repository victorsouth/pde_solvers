...
В ходе моделирования шаг по времени в общем случае не совпадает с шагом временных рядов, отсюда возникает интерполяции исторических данных. Надо синхронизировать временные ряды с шагом численного метода, который в общем случае может быть переменным.

![Временные ряды краевых условий и временная секта численного метода][timeseries_schema]

[timeseries_schema]: images/timeseries_schema.png "Временные ряды краевых условий и временная секта численного метода"  

<div align = "center">
</center><b>Временные ряды краевых условий и временная секта численного метода</b></center>
</div>
<br>

...
5. Чтобы определить высотные отметки соответствующих координат нового профиля применяется следующий метод:
    a. Для каждой точки определяется область притяжения - по половине сегмента в каждую сторону
    b. На области притяжения определяется максимальная высотная отметка и присваивается соответствующей координате
...
<br>
    ...
    c. Так как начальной и конечной точкам нового профиля будут соответствовать высотки начальной и конечной точек исходного профиля, область притяжения второй и последней точек полученного профиля будет составлять полтора сегмента

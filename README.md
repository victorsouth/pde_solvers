...
- `Layer` - тип слоя, содержащего профили плотности, вязкости и давления
...
Методы класса `isothermal_quasistatic_task_t` представляют собой все шаги гидравлического изотермического квазистационарного расчёта. Все действия производятся над полем buffer типа `isothermal_quasistatic_task_buffer_t`, в котором и хранятся результаты расчётов.
**Подготовительные шаги**
`.solve(initial_conditions)` - инициализация реологии в трубопроводе и начальный гидравлический расчёт, в качестве начальных условий на вход подаётся структура типа `isothermal_quasistatic_task_boundaries_t`

`.advance()` - циклический сдвиг индекса текущего слоя (см. [Идея буфера pde_solvers](#Идея-буфера-pde_solvers))

**Следующие действия должны повторяться на каждом шаге моделирования**
`.get_time_step_assuming_max_speed(v_max)` - если временной шаг моделирования не задан, тогда по значению максимальной скорости `v_max` течения потока в трубопроводе рассчитывается шаг по времени для `Cr = 1`

`.step(dt, boundaries)` - осуществляется расчёт шага движения партий(метод `.make_rheology_step(dt, boundaries)`) и гидравлический расчёт (метод - `.calc_pressure_layer(boundaries)`) для соответствующего шага по времени, в качестве краевых условий на вход подаётся структура типа `isothermal_quasistatic_task_boundaries_t`

`.advance()` - сдвигается индекс текущего слоя 

~~Я чего-то не понял или вывод не корректный? Выводится текущий слой уже после advance~~
`.print_all(dt, path)` - в файл выводятся профиля плотноти, вязкости, давления и дифференциального профиля давления на соотоветствующем шаге моделирования

`.get_buffer()` - при необходимости есть возможность вытянуть из класса поле buffer

### Проблемено-ориентированные абстракции
Методы класса `isothermal_quasistatic_task_t` оперируют и принимают на вход абстракции более низкого уровня.

`density_viscosity_cell_layer` и `density_viscosity_layer_moc` - проблемно-ориентированные слои для расчёта движений партий соответствиющим методом (см. [описание](Гидравлический-изотермический-квазистационарный-расчет)). Хранят внутри себя профили плотности и вязкости, а также профиль давления и дифференциальный профиль давления.

~~Какой в нём смысл?~~
`isothermal_quasistatic_task_buffer_t` - структура, хранящая в себе начальный профиль давления и [буфер](#Организация-хранения-расчетных-профилей-в-буферах-pde_solvers) для квазистационарного расчёта
...
Гидравлический изотермический квазистационарный расчёт реализован в методе `perform_quasistatic_simulation` в файле исследоавния `quick_with_quasistationary_model.h`

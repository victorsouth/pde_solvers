...
Чтобы доказать, что квазистационарная модель является более точной по сравнению со стационарной, проводится гидравлический расчёт работы трубопровода на данных с реального линейного участка. Таким образом, в качестве краевых условий выступали интерполированные значения показаний датчиков давления и расхода в начале и конце линейного участка, а также вязкости и плотности на входе ЛУ за период равный одному месяцу. 
Результат эксперимента – временной ряд отклонения расчётного давления от фактического в конце линейного участка

Перед тем как сравнить стационарную модель с квазистационарной выбирается наболее оптимальный вариант задания реологии в трубопроводе для стаца из следующих вариантов:
- Берётся реология из начала периода
- Используется текущая реология
- Берётся средняя реология за период

Для определения более точной модели рассматривается распределение отклонения расчётного давления от фактического. 

Так как квазистационарная модель точнее стационарной дополнительно проводятся эксперименты для определения влияния плотности и вязкости на расчёт квазистационарной модели. Проведится два эксперимента: в одном моделируется изменение вязкости, а в другом – изменение плотности. Второстепенный параметр рассчитывается так же как и в стационарной модели – берётся текущее значение

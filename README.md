...
Методы класса `isothermal_quasistatic_task_t` представляют собой все шаги гидравлического изотермического квазистационарного расчёта. Все действия производятся над полем buffer типа `isothermal_quasistatic_task_buffer_t`, в котором и хранятся результаты расчётов на текущем шаге моделирования.

...
struct quasistatic_printer {
    ...
    static void print_all(const time_t& t, const pipe_properties_t& pipe, const density_viscosity_quasi_layer<std::is_same<Solver, advection_moc_solver>::value ? false : true >& layer, std::string folder) {
...
TEST_F(IsothermalQuasistaticModelWithRealData, StationaryCurrentReology)
{
    ...
    perform_quasistatic_simulation<advection_moc_solver, quasistatic_printer<advection_moc_solver>>(
...
TEST_F(IsothermalQuasistaticModelWithRealData, StationaryMeanReology)
{
    ...
    auto& density_values = tag_data_initial[2].second;

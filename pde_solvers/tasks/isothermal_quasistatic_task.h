...
template <bool CellFlag>
struct density_viscosity_quasi_layer_PP {
    /// @brief Профиль плотности
    std::vector<double> density;
    /// @brief Профиль вязкости
    std::vector<double> viscosity;
    ...
    /// @brief Инициализация профилей
    /// @param point_count Количество точек
    density_viscosity_quasi_layer_PP(size_t point_count)
        : density(point_count - static_cast<int>(CellFlag))
        , viscosity(point_count - static_cast<int>(CellFlag))
        , specific(point_count)
        , pressure(point_count)
    ...
    /// @param layer Слой
    /// @return Обертка над составным слоем
    static quickest_ultimate_fv_wrapper<1> get_density_wrapper(density_viscosity_quasi_layer_PP& layer)
    {
        return quickest_ultimate_fv_wrapper<1>(layer.density, layer.specific);
    }
    ...
    /// @param layer Слой
    /// @return Обертка над составным слоем
    static quickest_ultimate_fv_wrapper<1> get_viscosity_wrapper(density_viscosity_quasi_layer_PP& layer)
    {
        return quickest_ultimate_fv_wrapper<1>(layer.viscosity, layer.specific);
    }
};

struct isothermal_quasistatic_PP_task_boundaries_t {
    ...
    /// @brief Конструктор по умолчанию
    isothermal_quasistatic_PP_task_boundaries_t() = default;

    /// @brief Конструктор краевых условий
    /// @param values Значения краевых условий
    isothermal_quasistatic_PP_task_boundaries_t(const std::vector<double>& values) {
        pressure_in = values[0];
        pressure_out = values[1];
        density = values[2];
        viscosity = values[3];
    }
};
...
template <typename Solver = advection_moc_solver>
class isothermal_quasistatic_PP_task_t {
public:
    /// @brief Тип слоя
    using layer_type = density_viscosity_quasi_layer_PP<std::is_same<Solver, advection_moc_solver>::value ? false : true>;
    /// @brief Тип буфера
    using buffer_type = ring_buffer_t<layer_type>;
    /// @brief Тип граничных условий
    using boundaries_type = isothermal_quasistatic_PP_task_boundaries_t;
private:
    // Модель трубы
    pipe_properties_t pipe;
    // Создаётся буфер, тип слоя которого определяется в зависимости от типа солвера
...
public:
    /// @brief Конструктор
    /// @param pipe Модель трубопровода
    isothermal_quasistatic_PP_task_t(const pipe_properties_t& pipe, QuasistaticModelType model_type = QuasistaticModelType::FullQuasi)
        : pipe(pipe)
        , buffer(2, pipe.profile.get_point_count())
        , model_type(model_type)
    {
    ...
    /// @brief Геттер для текущего слоя  
    density_viscosity_quasi_layer_PP<std::is_same<Solver, advection_moc_solver>::value ? false : true> get_current_layer() {
        return buffer.current();
    }
    ...
    /// Ставим по всей трубе реологию из initial_conditions, делаем гидравлический расчет
    /// @param initial_conditions Начальные условия
    void solve(const isothermal_quasistatic_PP_task_boundaries_t& initial_conditions)
    {
        // Количество точек
        size_t n = pipe.profile.get_point_count();
        ...
        Solve_PP test(initial_conditions, current, pipe);
        ...
        current.volume = result.argument;
...
private:
    ...
    /// @param dt Временной шаг моделирования
    /// @param boundaries Краевые условия
    void make_rheology_step(double dt, const isothermal_quasistatic_PP_task_boundaries_t& boundaries) {
        size_t n = pipe.profile.get_point_count();
        std::vector<double>Q_profile(n, buffer.current().volume/*boundaries.volumetric_flow*/); // задаем по трубе новый расход из временного ряда
    ...
    /// @brief Рассчёт профиля давления методом Эйлера (задача PP)
    /// @param boundaries Краевые условия
    void calc_pressure_layer(const isothermal_quasistatic_PP_task_boundaries_t& boundaries) {
        ...
        Solve_PP test(boundaries, current, pipe);
        fixed_solver_parameters_t<1, 0> parameters;
        // Создание структуры для записи результатов расчета
        fixed_solver_result_t<1> result;
        // Решение системы нелинейныйх уравнений <2> с помощью решателя Ньютона - Рафсона
...
public:
    ...
    /// @param dt временной шаг моделирования
    /// @param boundaries Краевые условие
    void step(double dt, const isothermal_quasistatic_PP_task_boundaries_t& boundaries) {
        make_rheology_step(dt, boundaries);
        calc_pressure_layer(boundaries);
    }
...
template <typename boundaries, typename layer_type>
class Solve_PP : public fixed_system_t<1> {
...
public:
    Solve_PP(const boundaries& bound, layer_type& current_layer, const pipe_properties_t& pipe)
        : bound(bound)
    ...
    var_type residuals(const var_type& x) {
        ...
        std::vector<double>& p_profile = current.pressure;
        int euler_direction = +1; // Задаем направление для Эйлера
        isothermal_pipe_PQ_parties_t pipeModel(pipe, current.density, current.viscosity, x, euler_direction);
    ...
    void set_parametrs(boundaries& bound, layer_type& current_layer) {
        this->bound = bound;
